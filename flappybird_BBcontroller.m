function flappybird_BBcontroller
...
%Proportional only controller for Flappy Bird.
    ...
    function error() % Function for calculating error
        ...
        GL2 = [128 177] - Tubes.VOffset(2) - 1; % Gap of 2nd Tube
        GL3 = [128 177] - Tubes.VOffset(3) - 1; % Gap of 3rd Tube
        SP(1) = (((GL1(2) - GL1(1))/2) + GL1(1) + 10); % setpoint for 1st Tube [adjusts the SP above the top section of the lower tube [Range:0-14]]
        SP(2) = (((GL2(2) - GL2(1))/2) + GL2(1) + 10); % setpoint for 2nd Tube
        SP(3) = (((GL3(2) - GL3(1))/2) + GL3(1) + 10); % setpoint for 3rd Tube
        P(SP);
    end
    function P(SP) % Function for calculating Control input
         if  Tubes.ScreenX(Tubes.FrontP) == 45-30 && flag == 1 % adjusts the timing at which the next setpoint is sent to the controller [Range:24-30]
                   updatesp = updatesp + 1;
         end
         if updatesp > 3
             updatesp = 1;
